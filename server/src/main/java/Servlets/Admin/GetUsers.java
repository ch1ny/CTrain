        ...
        if (jwt == null || jwt.equals("Bearer null")) {
            ...
            if (jwtVer == 0) {
                ...
                try {
                    ...
                    if (rs.next()) {
                        ps = conn.prepareStatement("SELECT COUNT(user_id) total FROM Users");
                        ps.execute();
                        rs = ps.getResultSet();
                        if (rs.next()) {
                            ...
                            ps = conn.prepareStatement("SELECT A.user_id, A.user_name, IF (B.user_id IS NULL , FALSE, TRUE) admin FROM ( SELECT user_id, user_name FROM Users LIMIT ?, ? ) A LEFT OUTER JOIN Admins B ON A.user_id = B.user_id");
                            ...
                            while (rs.next()) {
                                ...
                                map.put("admin", rs.getBoolean("admin"));
                                trains.add(map);
                            }
                            pw.write("{\"code\":0, \"data\": {\"total\":" + total + ", \"users\":" + JSON.toJSONString(trains) + "}}");
                        }
                    } else {
